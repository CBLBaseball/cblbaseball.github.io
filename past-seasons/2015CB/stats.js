...
headStandings = new Array("Division", "Wins", "Losses", " GB ", "Win Pct.", "Home", "Road");
headWildCard = new Array("Wins", "Losses", "Win Pct.", " GB ");
...
headLeaders = new Array ("Batting Average", "At Bats", "Runs Scored", "Hits",
						...
						"Hitting Streak", "Pinch Hit Bat Avg", "Slugging Pct", "On Base Pct",
...
function BoxStandings(sDivTitle, aDiv, columns)
{
	...
	var	bg = "TData2";				
	var url;
	dArray = new Array( 2,6,7,8,9,10 );		// display columns
...
function wildCardsToggle(button, idx) {
	if(button.value == 'Show WildCard') {
		document.getElementById("span"+idx).style.display = 'inline';
		button.value = 'Hide WildCard';	
	} else {
		document.getElementById("span"+idx).style.display = 'none';
		button.value = 'Show WildCard';
	}

}

function DisplayWildCards(subName, columns) {

	var dCount = eval(subName).length;
	var divName = new Array();
	var wcTeams = new Array();
	...
	var iGameUp = 0;
	var iGamesFirst = 0;
	var totWins, totLosses;
	var counter = 0;
	
	for(var i=0;i<eval(subName).length;i++) {
		divName[i] = subName + eval(subName)[i];
	}
	for (i = 0; i < dCount; i++) {
		tCount = eval(divName[i]).length;
		sortArray.length = 0;
		for (j = 0; j < tCount; j++) {
			sortArray.push(eval(divName[i])[j]);
		}
		sortArray.sort(sortStandings);
		sortArray.shift();
		wcTeams = wcTeams.concat(sortArray);
	}
	wcTeams.sort(sortStandings);

	document.write("<tr>");
	document.write("<td colspan='"+(columns+1)+"' align='center' class='BoxHead3' width='100%' nowrap>");
	document.write(subName +  " Wild Card Standings</td>");
	document.write("</tr>");
	// Header Row
	document.write("<tr>");
	document.write("<td align=left NoWrap  class='TSubHead2'>&nbsp;");
	...
	{
		document.write("<td align=center NoWrap  class='TSubHead2'>");
		document.write(headWildCard[i]+ "</td>");
	}
	document.write("</tr>");
	counter = (wildcardsThisMany > wcTeams.length) ? wcTeams.length : wildcardsThisMany;
	for(i=0;i<counter;i++) {
		team1 = eval(wcTeams[i]);
		iGamesUp = eval(team1)[6] - eval(team1)[7];
		if(i==0) {
			iGamesFirst = iGamesUp;
			GamesBack = "-";
		} else {
		...
		document.write(eval(team1)[1] + "</TD>");
		document.write("<td align='center'>"+eval(team1)[6]+"</td>");
		document.write("<td align='center'>"+eval(team1)[7]+"</td>");
		document.write("<td align='center'>"+eval(team1)[8]+"</td>");
		document.write("<td align='center'>"+GamesBack+"</td>");
		document.write("</tr>");
	}
}
